...
public class TrenMovimiento : MonoBehaviour
{
    ...
    void Start()
    {
        targetPosition = transform.position; // iniciar posici�n objetivo
    }
    ...
    void Update()
    {
        // Movimiento horizontal constante (puedes seguir reduci�ndolo si quieres)
        float horizontalSpeed = velocidad;

        float verticalDiff = Mathf.Abs(transform.position.y - targetPosition.y);
        if (verticalDiff > 0.01f)
            horizontalSpeed *= 0.85f; // opcional: m�s lento mientras sube/baja

        transform.position += Vector3.right * horizontalSpeed * Time.deltaTime;

        // <<< AQU� EST� LA MAGIA >>>
        // Suavizado natural con curva de aceleraci�n/desaceleraci�n
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("ResetPoint"))
        {
            ...
            // Reseteamos la decisi�n correctamente
            gameManager.nuevaDecision = 0;
            gameManager.decision = 0; // <- Esto es clave

            // Animaci�n y sonido seg�n el estado previo
